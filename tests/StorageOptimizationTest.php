...
class StorageOptimizationTest extends TestCase
{
    ...
    {
        $this->assertEquals('local', $this->avatar->storageDisk);
        $this->assertEquals('test-avatars', $this->avatar->storageDirectory);
        $this->assertEquals(100, $this->avatar->maxStorageSize);
        $this->assertEquals(7, $this->avatar->maxFileAge);
    }
    ...
    public function testConfigureStorage()
    {
        ...
        $this->assertEquals('s3', $this->avatar->storageDisk);
        $this->assertEquals('avatars', $this->avatar->storageDirectory);
        $this->assertEquals(500, $this->avatar->maxStorageSize);
    }
    ...
    public function testSetCompressionEnabled()
    {
        ...
        $this->assertInstanceOf(get_class($this->avatar), $result);
        $this->assertFalse($this->avatar->compressionEnabled);
        
        $this->avatar->setCompressionEnabled(true);
        $this->assertTrue($this->avatar->compressionEnabled);
    }
    ...
    public function testSetMaxFileAge()
    {
        ...
        $this->assertInstanceOf(get_class($this->avatar), $result);
        $this->assertEquals(30, $this->avatar->maxFileAge);
    }
    ...
    public function testLoadStorageMetrics()
    {
        ...
        $this->assertIsArray($this->avatar->storageMetrics);
        $this->assertArrayHasKey('total_files', $this->avatar->storageMetrics);
        $this->assertArrayHasKey('total_size', $this->avatar->storageMetrics);
        $this->assertArrayHasKey('formats', $this->avatar->storageMetrics);
        $this->assertArrayHasKey('last_updated', $this->avatar->storageMetrics);
    }
    ...
    public function testUpdateStorageMetrics()
    {
        ...
        $initialFiles = $this->avatar->storageMetrics['total_files'] ?? 0;
        
        // Mock a file update
        $method->invoke($this->avatar, 'test/path.png', 'png');
        
        $this->assertEquals($initialFiles + 1, $this->avatar->storageMetrics['total_files']);
        $this->assertArrayHasKey('png', $this->avatar->storageMetrics['formats']);
        $this->assertNotNull($this->avatar->storageMetrics['last_updated']);
    }
